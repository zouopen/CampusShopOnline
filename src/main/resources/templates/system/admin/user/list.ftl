        ...
        security_tab = $('#security_tab').DataTable({
            ...
            "columnDefs": [
                ...
                },
                {
                    ...
                    "render": function (data) {
                        var btn = "";
                        btn = '<a class="btn btn-xs btn-primary" target="modal" modal="lg" href="/user/view?id=' + data.id + '">查看</a> &nbsp;'
                                +'<a class="btn btn-xs btn-info" onclick="securityToListAjax();" data-title="修改" target="modal" modal="lg" href="/user/edit?id='+data.id+'">修改</a> &nbsp;'
                                +'<a class="btn btn-xs btn-info btn-pinterest" onclick="securityToListAjax();" target="modal" modal="lg" href="/user/goResetPwd?id='+ data.id+ '">重置密码</a> &nbsp;'
                                +'<a class="btn btn-xs btn-default btn-pinterest" callback="securityReload();" data-body="确认要删除吗？" target="ajaxTodo" href="/user/delete?id='+ data.id + '">删除</a>';
                        return btn;
                    }
                }]
